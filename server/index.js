...
// Details of all tools 
app.get('/tools', (req, res) => {
  res.json(tools);
});

// To provide all the list of the tools marked as favourite 
app.get('/favorites', (req, res) => {
  res.json(favorites);
});

// Adding tools to the favourite 
app.post('/favorites/:id', (req, res) => {
  const id = parseInt(req.params.id);
  const tool = tools.find(t => t.id === id);
  if (tool && !favorites.some(f => f.id === id)) {
    favorites.push(tool);
  }
  res.json(favorites);
});

// Deleting tools from the favourite list 
